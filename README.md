...
- **R7** — метрики на отложенных периодах: январь 2026 nMAE 15,5 %, февраль 2025 — 17,8 % (таблица ниже).
...
| Вариант | MAE |
|---|---|
| A — персистентность (среднее за последние 24 ч) | 0,307 |
| B — фиксированный пайплайн: кривая мощности, без проверок и пересчёта | 0,195 |
| C — только модель: бустинг в момент t0, без пересчёта | 0,155 |
| D — агент: проверки, фолбэки, пересчёт в t0+12 ч | **0,151** |
...
|---|---|---|---|---|
| взять бустинг вместо кривой мощности | 31 из 31 | −0,0398 | [−0,0556; −0,0244] | 23 |
| пересчёт в t0+12 ч по свежим прогонам | 31 из 31 | −0,0063 | [−0,0152; +0,0018] | 19 |
...
| выброс 100 м/с в основном источнике | 48 ч, переключился на `gfs_seamless` | 48 ч, но **6 часов посчитаны по битому входу** |
...
Агент выдал полный горизонт в **4 случаях из 4** и в каждом записал решение и причину в `runs/<run_id>/agent_log.jsonl`. Фиксированный пайплайн либо теряет часы, либо — что хуже — молча считает прогноз по испорченному входу.
...
| **Градиентный бустинг (медиана)** | **0,155** | **0,178** |

Коридор p10–p90 после конформной калибровки покрывает 83,3 % и 84,1 % часов при цели 80 %. Результаты — `outputs/metrics/holdout_2026-01.json` и `holdout_2025-02.json`.

**Во что точность превращается в деньгах.** Каждый час, где факт вышел за коридор ±5 % от заявки, оплачивается как дисбаланс. Верхняя оценка платы за месяц для ВЭС 5 МВт при 22,68 ₸/кВт·ч, режим нового договора:

| Модель | Январь 2026 | Февраль 2025 |
|---|---|---|
| Персистентность | 7,76 млн ₸ | 7,67 млн ₸ |
| Кривая мощности | 4,93 млн ₸ | 4,69 млн ₸ |
| **Градиентный бустинг** | **3,93 млн ₸** | **4,03 млн ₸** |
| **Экономия против персистентности** | **3,83 млн ₸** | **3,64 млн ₸** |
...
| R7 | Метрики на отложенных периодах против базовых моделей | `app/evaluate.py` | `outputs/metrics/holdout_2026-01.json` (nMAE 15,5 %) и `holdout_2025-02.json` (17,8 %) | ✅ |
