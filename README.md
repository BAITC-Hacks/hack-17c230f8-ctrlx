...
- **Демо:** платформа — https://ctrlx-hackalem.vercel.app, API — https://windcast-api-production.up.railway.app/docs (без логина). Локальный запуск — раздел «Установка и запуск», ключи не нужны.
...
SHA-256 файлов в репозитории (переводы строк LF, как их отдаёт `git clone`); у оригиналов с Google Drive (CRLF) суммы `d82def7e56c0a1eed3f2f68eb29fd66e4299999921c9703720a880a5cf7d6703` и `16a844db949562290c96a86efa20178a71540ff6252275c003db4715edb9434a`, данные те же.
...
- API агента: https://windcast-api-production.up.railway.app — Swagger на `/docs`, например `GET /api/issues`, `POST /api/run` (Docker-образ из `Dockerfile` на Railway).
- В футере платформы — хэш коммита, из которого собран деплой. Локальный запуск без аккаунтов — раздел «Установка и запуск».
