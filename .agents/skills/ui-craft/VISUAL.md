# Visual direction — original text

> Original text of Anthropic's `frontend-design` skill (Apache-2.0, see `LICENSE-frontend-design.txt`), kept here for depth; the only change is this header. In the hackathon the "client" is the task: take subject, audience and the screen's job from `docs/TASK.md` instead of waiting for confirmation.
