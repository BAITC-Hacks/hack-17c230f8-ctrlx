...
- **Коридор p10–p90:** квантили бустинга без калибровки покрывают 70–76 % случаев вместо 80 % **[E4]**. Калибровка CQR на последних 60 днях до выпуска исправляет это: в коде репозитория покрытие 83,3 % (январь 2026) и 84,1 % (февраль 2025) при средней ширине 0,52 и 0,60 **[R]**.
...
3. **Измеренный эффект решений.** Пересчёт при свежем прогоне на часах, которые ещё разрешено корректировать (не раньше чем за 2 ч): январь 2026 — MAE 0,161 → 0,154, а на часах, где поле погоды реально сменилось, 0,176 → 0,165; февраль 2025 — 0,173 → 0,170 и 0,185 → 0,180 **[R]**. Эффект есть в обоих месяцах, но разный по силе, поэтому агент показывает фактический сдвиг каждого пересчёта, а не обещанный процент.
...
Python 3.12, uv, pandas, pyarrow, scikit-learn (HistGradientBoosting), FastAPI, pydantic, pytest, ruff; OpenAI-совместимый клиент для необязательного LLM. Страница — HTML + Chart.js без сборки. Всё работает на обычном ноутбуке без GPU, без ключей и без сети (офлайн-кэш). Контракт — `app/schemas.py`; конфиг и правило утечки — `app/config.py`.
...
## 15. Статус реализации (на 15:35, 23.09.2026)

| Компонент | Статус |
|---|---|
| Контракт, конфиг, CLI, зоны, задачи | в репозитории |
| Погода: загрузка, кэш best_match и gfs_seamless, выбор по правилу, тесты (Амирхан) | в репозитории; обе координаты и живой режим — в работе |
| История `app/data.py` (фиксированный UTC+5) | в репозитории |
| Модели, обучение, проверки на январе 2026 и феврале 2025 | в репозитории, `models/windcast.pkl` 4,5 МБ |
| Агент: оркестратор, инструменты, журнал, самокоррекция, пересчёт, самопроверка, сводка | в репозитории |
| `backtest`: 28 выпусков, `february_2026.csv` (672 часа) | в репозитории, 4 с без ключей |
| Тесты | 30 зелёных, включая «отравленный» тест и детерминизм |
| API и страница (Ансар) | в репозитории, работают на реальных выпусках |
| README, smoke (Ансар) | в репозитории, обновляется под реальные цифры |
| Реплей января с бухгалтерией решений, абляция, отказоустойчивость, LLM-сводка | P1, не сделано |
