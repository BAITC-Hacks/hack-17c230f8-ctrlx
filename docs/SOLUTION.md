...
- Цель «Самрук-Энерго» по пресс-релизу Минэнерго от 20.08.2026 — «повысить точность прогнозов генерации до 80 %», запуск ИИ-решения в конце 2026 года; метрика не определена **[Д: gov.kz, inform.kz]**. Прямое сравнение с этой целью невозможно без её определения. Наша измеренная nMAE — **15,55 %** на январе 2026 и **17,71 %** на феврале 2025; у персистентности — 30,66 % и 33,86 % **[R]**. Дополнительный показатель `1 − MAE` равен примерно 84,5 % и 82,3 %; это не доля правильно угаданных часов.
...
- **Ограничение CQR:** временные наблюдения коррелированы; универсальная гарантия покрытия 80 % не заявляется. После пересчёта покрытие — 81,6 % и 82,0 %, средняя ширина — 0,501 и 0,590 номинала. Коридор относится к станции, а не к отдельным турбинам. Расхождение NWP-моделей — отдельный флаг риска, не источник границ CQR.
...
| **Градиентный бустинг** | **0,1555** | **0,1771** |

Команды: `uv run python -m app.cli evaluate --holdout 2026-01` и `--holdout 2025-02`. Обучение заканчивается строго до начала проверяемого месяца. Январь содержит 1 464 пары «выпуск × целевой час» на 744 уникальных часах; февраль — 1 308 пар на 666 уникальных часах. Шесть часов 28.02.2025 без факта обеих турбин исключены. GBM по D+1/D+2: январь 0,1509/0,1602, февраль 0,1785/0,1756. Skill относительно персистентности — 49,3 % и 47,7 %, относительно кривой мощности — 20,4 % и 14,4 %.
...
После исправления цели станции и CQR обе проверки выполнены повторно. Источники — `outputs/evidence/replay_2026-01.json` и `outputs/evidence/faults.json`.

| Вариант replay, январь 2026 | MAE |
|---|---:|
| A: персистентность | 0,3066 |
| B: фиксированная кривая мощности | 0,1953 |
| C: GBM в момент выпуска | 0,1555 |
| D: опубликованный план оркестратора | **0,1536** |

31 выпуск, 1 464 пары «выпуск × целевой час». В варианте D первые сутки обновляются в 12:00, заявка D+2 остаётся из первоначального выпуска. Проектный выбор GBM против кривой: средняя по выпускам ΔMAE −0,0407, 95 % интервал [−0,0563; −0,0255], 24 улучшения из 31. Пересчёт сработал 31 раз, улучшил результат в 18 выпусках: ΔMAE −0,0064 на затронутых часах, интервал [−0,0152; +0,0017]. **Интервал пересчёта включает ноль: устойчивый выигрыш не доказан.** Эти средние по выпускам/затронутым часам не обязаны совпадать с разностью MAE всей таблицы.
...
| Вероятностный прогноз станции | CQR с отдельными обучающими признаками; измеренное покрытие 81,6/82,0 % при цели 80 % |
...
| История, модели, обучение, проверки на январе 2026 и феврале 2025 | переобучены после исправлений; nMAE 15,55/17,71 %, детали в `METRICS.md` |
...
| Реплей января, журнал решений и сценарии сбоев | обновлены: MAE плана 0,1536; пять сбоев из пяти дают 48 валидных часов |
...
| «Как определена точность?» | Основная метрика — nMAE: 15,55 % в январе и 17,71 % в феврале. `1 − MAE` — дополнительное преобразование, не доля правильно угаданных часов. |
