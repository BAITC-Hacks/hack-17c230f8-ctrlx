...
Команда: `uv run python -m app.cli evaluate --holdout 2026-01` (и `2025-02`); модель каждый раз обучается строго до начала месяца. Skill к персистентности — 50 % (январь 2026) и 48 % (февраль 2025). По горизонту на январе: сутки D+1 — 0,150, сутки D+2 — 0,158 **[R]**. Помесячная проверка за 17 месяцев сделана в эксперименте на LightGBM: бустинг лучше кривой мощности в 16 из 17 месяцев **[E]**.
...
2. **Реплей января 2026.** Тот же агент заново проживает месяц, где факт известен: модель обучена строго до 01.01.2026, факт приходит день за днём, 31 выпуск.
   ...
   | C. Только модель: бустинг в момент выпуска, без пересчёта | 0,154 |
   | **D. Агент: проверки, запасные ветки, пересчёт в 12:00 (опубликованный план)** | **0,150** |
