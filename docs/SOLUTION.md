...
6. **«Отравленный» тест.** Все наблюдения после момента выпуска и все недопустимые поля погоды (более свежие `previous_dayN`) подменяются на мусор (999 или NaN). Прогноз обязан остаться побайтно тем же. Если хоть одно число изменилось — значит, код подглядывает в будущее, и тест падает.
...
nMAE и nRMSE (доля номинала), bias, skill к персистентности и климатологии, по горизонтам «сутки D+1» и «сутки D+2»; для коридора — покрытие p10–p90 (цель 80 %), средняя ширина и pinball loss; «точность» = 1 − nMAE только с явной формулой.

Метрики на языке регулятора: доля часов, где отклонение от плана укладывается в ±5 % (коридор п. 132 Правил БР), ±20 % (порог для прочих субъектов, п. 90) и ±30 % (ВИЭ с накопителем, п. 132-1); объём отклонений в МВт·ч; стоимость дисбаланса в тенге в трёх режимах договора (старый договор РФЦ — 0 для производителя, новый — 0,3 × цена за кВт·ч вне ±5 %, прочие субъекты). Оценка сверху для января 2026 на 5 МВт при цене 22,68 тг/кВт·ч, если все часы вне коридора: LightGBM ≈ 3,8 млн тг в месяц, кривая мощности ≈ 4,9 млн, персистентность ≈ 7,8 млн **[E + расчёт]**. На фактах реплея оценка заменяется точным расчётом по часам.

Качество предупреждений тоже измеряется, иначе это не «умные флаги»: для рамп — доля пойманных и ложных тревог (POD, FAR) с допуском ±2 ч на реплее; для «низкого доверия» — MAE в помеченных часах против остальных.
...
2. **Бухгалтерия решений с альтернативой.** Для каждой развилки агент хранит и выбранную, и отвергнутую ветку. На реплее января 2026, где факт известен, он сам считает, насколько выбор улучшил или ухудшил MAE, и пишет итог в `outputs/metrics/decisions_2026-01.json`: тип решения, сколько раз сработало, средний эффект, доля побед. Если правило в среднем вредит, это видно, и мы его выключаем.
3. **Измеренный эффект решений** на реплее января 2026 (факт известен): пересчёт при свежем прогоне снижает MAE LightGBM 0,153 → 0,150 и RMSE на 4,4 %; на часах, где поле погоды реально сменилось, 0,174 → 0,165 **[E11]**. В феврале 2025 эффекта нет (0,168 → 0,169) — показываем это честно: польза пересчёта зависит от месяца.
4. **Абляция одной командой** (план, раздел 14): персистентность → фиксированный конвейер (кривая мощности на day2, без проверок и пересчёта) → агент.
5. **Отказоустойчивость (fault injection):** подаём пропуски часов погоды, NaN, ветер 100 м/с, недоступный основной источник. Фиксированный конвейер падает или выдаёт мусор, агент восстанавливается через запасные ветки и пишет причину.
6. **Тесты:** правило утечки на каждой строке, «отравленный» тест (раздел 6), детерминизм (два запуска без ключа дают одинаковый CSV), пересчёт появляется в t0 + 12 ч, LLM не может записать число, которого нет в фактах.
...
| Мустафа | оркестратор с шагами 1–10 и развилками раздела 8.3, `backtest` на 28 выпусков, сшитый февральский файл, `PUBLISH_DELAY_H = 7` (сделано) | `backtest` без ключа < 5 мин, журналы с решениями |
| Мустафа | «отравленный» тест утечки и тест детерминизма (`tests/test_agent.py`) | `uv run pytest -q` зелёный |
...
| Загрузка истории `app/data.py`, задержка публикации 7 ч | в репозитории (e112a9a) |
