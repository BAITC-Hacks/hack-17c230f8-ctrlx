...
**Как читать.** Каждое число помечено источником: **[R]** — посчитано кодом этого репозитория (`outputs/metrics/*.json`, команда `evaluate`), **[E]** — предварительный эксперимент на наших данных (скрипты вне репозитория; независимо воспроизвести его по этому репозиторию нельзя), **[Д]** — документ или статья, прочитанные командой, **[Г]** — гипотеза или допущение. Раздел 15 — статус реализации на момент написания: что уже в репозитории, а что только спроектировано.

---

## 1. Суть в одном абзаце

Мы строим агента, который каждый день тестового периода «просыпается» в конце суток D, как дежурный диспетчер ВЭС, и выпускает почасовой прогноз выработки на следующие 48 часов. Погоду он берёт сам, по координатам станции, из открытого архива прогнозов Open-Meteo, и выбирает прогоны, допустимые по принятому консервативному правилу с задержкой публикации 7 часов. Тесты и поля каждой строки проверяют соблюдение этого правила; фактические исторические отметки публикации каждого прогона не сохранены. Числа считают проверенные модели: градиентный бустинг (HistGradientBoosting из scikit-learn) с калиброванным коридором p10–p90, а запасная модель — кривая мощности. Агент проверяет входы и выход, переключается на запасные варианты, пересчитывает прогноз, когда выходит более свежий прогон погоды, и объясняет диспетчеру по-русски, что изменилось и насколько прогнозу можно верить. Каждое решение оркестратора записано с причиной. Качество модели проверяется на январе 2026 и феврале 2025; вклад пересчёта оценивается отдельно. Необязательный LLM-супервизор динамически выбирает диагностические инструменты после расчёта выпуска (раздел 8.5).
...
- Цель «Самрук-Энерго» по пресс-релизу Минэнерго от 20.08.2026 — «повысить точность прогнозов генерации до 80 %», запуск ИИ-решения в конце 2026 года; метрика не определена **[Д: gov.kz, inform.kz]**. Прямое сравнение с этой целью невозможно без её определения. Наша измеренная nMAE — **15,36 %** на январе 2026 и **17,78 %** на феврале 2025; у персистентности — 30,66 % и 33,86 % **[R]**. Дополнительный показатель `1 − MAE` равен примерно 84,6 % и 82,2 %; это не доля правильно угаданных часов.
...
- Для самой «Нурлы» прямой штраф может быть близок к нулю (режим её договора по открытым данным не установлен). Ценность масштабируется на новые проекты группы: ВЭС 1 ГВт с Masdar и ВЭС 1 ГВт с CPIH строятся по новым правилам **[Д]**. Перенос на другой парк требует его истории, координат, номинальной мощности, проверки модели и отдельной оценки качества. Линейное масштабирование сценарной стоимости не доказывает переносимость модели.
...
- Корреляция ветра и мощности 0,94; турбины между собой: ветер 0,99, мощность 0,96. Цель обучения станции — среднее двух равномощных турбин, только когда известны обе; индивидуальные цели сохраняются и при пропуске соседней турбины. Медиана станции прогнозируется отдельной моделью и не обязана равняться среднему прогнозных медиан турбин.
...
Прогон с инициализацией `init` допустим по нашему правилу, если `init + задержка публикации ≤ t0`. Принимаем задержку **7 часов**; селектор оставляет дополнительный запас за счёт выбора более старого поля. Журнал хранит расчётную оценку доступности и запаса. Это не историческая отметка фактической публикации и не гарантия максимальной задержки поставщика. В предварительном исследовании наблюдались задержки ECMWF IFS HRES 7,0 ч, IFS 0,25° 7,8 ч, GFS 5,5–6,5 ч, ICON 3,8 ч **[E: meta.json Open-Meteo]**; эти наблюдения не заменяют журнал за весь период. Следующий шаг — сохранять конкретные прогоны, времена инициализации и получения данных.
...
## 6. Честность во времени: проверяемые ограничения и допущения

Кейс требует воспроизводить прошлое. Ограничения обучения и выбора полей проверяются кодом; задержка публикации остаётся явно названным допущением источника (раздел 5.3).
...
### 7.2. Как обучаем
- **Симуляция выпусков:** исторические выпуски используют тот же селектор погоды, что и тест. Production-модель после исправлений обучена на 30 886 строках станции из 657 выпусков; история ограничена `train_end = 2026-01-31 19:00 UTC` (01.02 00:00 Алматы).
- **Признаки:** ветер 100 м и его куб, ветер 10 м, сдвиг ветра, порывы, sin/cos направления, температура, 1/T, sin/cos часа, месяц, опережение, поле, `nwp_epoch`, выход кривой мощности.
- **Гиперпараметры:** learning_rate 0,03, 500 итераций, 15 листьев, минимум 50 наблюдений в листе. Изменения калибровки не подбирают эти параметры по результату проверки.
- **Разделение CQR:** квантили имеют собственные кривые мощности, обученные до последних 60 дней истории. Целевые часы calibration-периода исключены из обучения квантилей, включая пересечение соседних 48-часовых выпусков. Калибровочный сдвиг — конечновыборочная порядковая статистика. Тест с подменой calibration-меток проверяет изоляцию признаков и обучения квантилей.
- **Ограничение CQR:** временные наблюдения коррелированы; универсальная гарантия покрытия 80 % не заявляется. После пересчёта покрытие — 80,5 % и 82,0 %, средняя ширина — 0,491 и 0,582 номинала. Коридор относится к станции, а не к отдельным турбинам. Расхождение NWP-моделей — отдельный флаг риска, не источник границ CQR.
- **Воспроизводимость между платформами:** все числовые признаки округляются до 12 десятичных знаков общей функцией обучения и прогноза. Это устраняет обнаруженные переключения веток GBM из-за одного ULP в тригонометрии Windows/macOS; модель и артефакты после изменения переобучены. Проверка сдвигом `sin`/`cos`/интерполяции на ±1 ULP дала побитно одинаковые прогнозы на 2 296 строках. Отдельная проверка на Windows/Linux отражается в `TESTING.md`.
- **Почему scikit-learn:** используем HistGradientBoosting для воспроизводимой установки на поддерживаемых платформах. Предварительные эксперименты с LightGBM не являются метриками текущего артефакта.

### 7.3. Результаты после повторного обучения 23.09.2026

MAE — доля номинальной мощности; `nMAE = 100 × MAE`.

| Модель | Январь 2026 [R] | Февраль 2025 [R] |
|---|---:|---:|
| Персистентность | 0,3066 | 0,3386 |
| Климатология | 0,3039 | 0,3205 |
| Кривая мощности | 0,1953 | 0,2070 |
| **Градиентный бустинг** | **0,1536** | **0,1778** |

Команды: `uv run python -m app.cli evaluate --holdout 2026-01` и `--holdout 2025-02`. Обучение заканчивается строго до начала проверяемого месяца. Январь содержит 1 464 пары «выпуск × целевой час» на 744 уникальных часах; февраль — 1 308 пар на 666 уникальных часах. Шесть часов 28.02.2025 без факта обеих турбин исключены. GBM по D+1/D+2: январь 0,1491/0,1582, февраль 0,1796/0,1759. Skill относительно персистентности — 49,9 % и 47,5 %, относительно кривой мощности — 21,4 % и 14,1 %.

На этих двух периодах ранее принимались проектные решения, поэтому это валидация, а не полностью независимый тест выбора подхода. Предварительная проверка за 17 месяцев выполнена вне репозитория на LightGBM и не доказывает качество текущей модели. Независимую точность февраля 2026 здесь измерить нельзя: факта нет.

Точные границы, число наблюдений, время команд и происхождение артефактов — [METRICS.md](METRICS.md).
...
`evaluate` считает MAE, RMSE, nMAE, bias и skill относительно эталонов по обоим горизонтам; покрытие и ширину CQR; сравнение исходного прогноза с расчётом по более свежей погоде. Последнее сравнение не заменяет проверку окончательно опубликованного плана: её делает `replay` с проверками оркестратора.

Сценарная стоимость считается по часам первоначальной заявки D+2 (опережение 24–47 ч): сумма **всех абсолютных ошибок**, умноженная на 5 МВт, 1000, 22,68 тг/кВт·ч и 0,3. Расчёт не освобождает часы внутри ±5 % и не моделирует договор или взаимозачёт; это условная верхняя оценка, не реальный счёт. Январь: GBM 3,88 млн тг, кривая 5,10 млн, персистентность 7,44 млн. Февраль: 3,84 / 4,48 / 7,81 млн соответственно **[R]**. Оценены 720 и 642 часа заявки: первые сутки каждого месяца не покрыты исходными выпусками D+2, ещё шесть февральских часов не имеют факта; это не расчёт за полный календарный месяц.

Отдельно доля часов заявки с ошибкой не выше 5/20/30 % самого плана — 9,6/24,0/31,7 % в январе и 6,4/19,3/26,6 % в феврале. Это другая нормировка, не nMAE.

В развитие отнесены pinball loss, CRPS, качество предупреждений и моделирование конкретных договоров.
...
Числовое ядро — **детерминированный адаптивный workflow**: программа получает данные, проверяет их, выбирает резервные ветки, пересчитывает прогноз и записывает причины. Последовательность допустимых действий и пороги заданы кодом. Это автономное выполнение цикла ТЗ; LLM не управляет числовой моделью.

По `--supervise` подключается отдельный **диагностический LLM-агент**: модель выбирает native function calls `quality`, `weather`, `revisions`, получает их результаты и завершает разбор через `finish`. Он работает после записи CSV, оставляет решение `accepted` или `review` и не редактирует прогноз. Это не блокировка публикации и не отправка заявки на рынок.

Разделение — наш инженерный выбор для проверяемости. Улучшение числовой точности от LLM не измерялось и не заявляется. Ограничения, запасной режим и способ проверить настоящий журнал вызовов — [AGENTIC.md](AGENTIC.md).
...
flowchart TB
  ...
  LLM[LLM-сводка, по желанию] -.-> O
  O --> SUP[Необязательный LLM-супервизор<br/>инструменты диагностики → accepted/review]
...
- **Без ключа** числовой цикл работает самостоятельно; сводка шаблонная, supervisor использует детерминированное решение.
- **`--llm`** включает русскую текстовую сводку по посчитанным агрегатам. Проверка чисел отбрасывает значения вне набора фактов, но не доказывает семантическую правильность каждой фразы.
- **`--supervise`** запускает диагностический цикл после числового выпуска: инструменты → наблюдения → `accepted`/`review`. Сведения сохраняются в `supervisor.json`; только `mode=llm` с непустым журналом вызовов подтверждает настоящий LLM-прогон. Тестовые моки и `mode=demo` таким доказательством не являются.
- LLM не задаёт значения выработки, даты, квантили, метрики и пороги. Предупреждения или резервный прогноз принудительно требуют разбора, даже если модель предлагает принятие.
- OpenAI или совместимый NVIDIA endpoint выбирается настройками; нужны модель с поддержкой tools и локальный ключ. Ошибки провайдера не меняют числовой прогноз.

```bash
uv run python -m app.cli forecast --issue 2026-01-31 --llm --supervise --demo-dir runs/llm_demo
```

Настройка и точные пути доказательств — [AGENTIC.md](AGENTIC.md). Сейчас локально используется `DEMO_MODE=1`; пользователь отложил API-вызовы, поэтому настоящий LLM-прогон не подтверждён. Вызов API сам по себе не свидетельствует об улучшении точности.
...
## 9. Как проверяем вклад оркестратора

```bash
uv run python -m app.cli replay --month 2026-01
uv run python -m app.cli faults
```

`replay` обучает модель до января, выполняет 31 выпуск и отдельно сравнивает персистентность, фиксированную кривую, GBM и опубликованный агентом план. Журнал решений отделяет проектный выбор модели от эффекта пересчёта. Доверительные интервалы оцениваются бутстрэпом по выпускам; соседние выпуски имеют общие целевые часы, поэтому выводы по одному месяцу ограничены.

После исправления цели станции, CQR и нормализации признаков обе проверки выполнены повторно. Источники — `outputs/evidence/replay_2026-01.json` и `outputs/evidence/faults.json`.

| Вариант replay, январь 2026 | MAE |
|---|---:|
| A: персистентность | 0,3066 |
| B: фиксированная кривая мощности | 0,1953 |
| C: GBM в момент выпуска | 0,1536 |
| D: опубликованный план оркестратора | **0,1519** |

31 выпуск, 1 464 пары «выпуск × целевой час». В варианте D первые сутки обновляются в 12:00, заявка D+2 остаётся из первоначального выпуска. Проектный выбор GBM против кривой: средняя по выпускам ΔMAE −0,0427, 95 % интервал [−0,0582; −0,0272], 25 улучшений из 31. Пересчёт сработал 31 раз, улучшил результат в 18 выпусках: ΔMAE −0,0060 на затронутых часах, интервал [−0,0143; +0,0016]. **Интервал пересчёта включает ноль: устойчивый выигрыш не доказан.** Эти средние по выпускам/затронутым часам не обязаны совпадать с разностью MAE всей таблицы.

При пяти искусственных сбоях агент сохранил 48 валидных часов в каждом случае: три перехода на GFS, один выбор более старого поля и один переход на климатологию. Подробные значения и ограничения — [METRICS.md](METRICS.md).

`faults` проверяет пропуски, невозможный ветер, повреждение свежего поля и отказ источников. Полный горизонт и журнал решения подтверждают устойчивость сценария; без факта они не доказывают точность аварийного прогноза.

Тесты дополнительно проверяют допустимость погоды по каждой строке, независимость от недоступных полей, временную изоляцию CQR и состав журнала. Улучшение от модели, вклад пересчёта и качество работы LLM — три разные проверки.
...
## 12. Обоснованные особенности решения

| Решение | Проверяемое основание |
|---|---|
| Явный момент доступности входов | Единое правило выбора `previous_dayN`, поля происхождения и тесты; историческая задержка остаётся допущением |
| Разделение прогноза и управления | Числа считает ML; оркестратор проверяет и выбирает резерв; LLM-супервизор разбирает готовый выпуск |
| Вероятностный прогноз станции | CQR с отдельными обучающими признаками; измеренное покрытие 80,5/82,0 % при цели 80 % |
| Воспроизводимость | Данные, кэш, модель, команды обучения/оценки и границы выборок в репозитории |
| Практический результат | Почасовой план D+1, черновик заявки D+2, изменения и объяснения для диспетчера |
| Ограниченные утверждения | Валидационные месяцы названы явно; аварийная полнота и улучшение MAE не смешиваются; финансовая оценка сценарная |

Это особенности реализации, а не утверждение мировой новизны или сравнение с неизвестными решениями других команд.
...
## 14. Первоначальный план работ и путь развития

Ниже сохранён план распределения работ команды; это не итоговая приёмка. Текущий статус — раздел 15 и `docs/TESTING.md`.

### P0 — первоначальная цель к 16:30
| Кто | Что | Статус |
|---|---|---|
...
**Первоначальный порядок сокращения необязательного объёма:** сначала LLM-сводка, затем отказоустойчивость, затем часть предупреждений, затем CQR. Никогда не срезаем: задержку 7 ч и тест утечки, 28 выпусков и `february_2026.csv`, проверку на январе 2026 против эталонов, README из трёх команд.
...
| История, модели, обучение, проверки на январе 2026 и феврале 2025 | переобучены после исправлений; nMAE 15,36/17,78 %, детали в `METRICS.md` |
...
| `backtest`: 28 выпусков, `february_2026.csv` (672 часа) | повторно сформирован после новой модели; 28 файлов и 672 строки проверены |
| Реплей января, журнал решений и сценарии сбоев | обновлены: MAE плана 0,1519; пять сбоев из пяти дают 48 валидных часов |
| Тесты | последний полный прогон: 155 passed за 8,09 с; версия и итоговая проверка — в `TESTING.md` |
| API и страница (Ансар) | в репозитории, работают на реальных выпусках |
| Docker-образ | содержит модель и результаты |
| LLM-сводка и отдельный supervisor с native function calls | код и локальные тесты сохранены; локальный режим `DEMO_MODE=1`, живой API-прогон отложен пользователем и не подтверждён |
| «Спросить агента» (Амирхан) | логика `app/ask.py` отвечает по журналу выпуска, каждое число сверено с фактами, без ключа — шаблон; покрыта `tests/test_ask.py`; в интерфейс не выведена до фриза — развитие |

## 16. Риски и неудобные вопросы жюри

| Вопрос или риск | Ответ |
|---|---|
| «Где именно агентность?» | Числовой контур — детерминированный адаптивный workflow. Необязательный LLM-супервизор динамически вызывает инструменты диагностики после выпуска; границы и доказательства разделены (раздел 8.1). |
| «Есть ли утечка будущего?» | Временные границы обучения и правило выбора погоды проверяются тестами. Задержка 7 ч — допущение; точные исторические времена публикации каждого прогона не сохранены. |
| «Почему не ERA5 или Historical Forecast?» | Это факт или почти анализ, известный позже, — запрещено условием кейса. |
| «Как определена точность?» | Основная метрика — nMAE: 15,36 % в январе и 17,78 % в феврале. `1 − MAE` — дополнительное преобразование, не доля правильно угаданных часов. |
