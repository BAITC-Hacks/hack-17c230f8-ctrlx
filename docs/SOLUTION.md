...
3. **Бухгалтерия решений с доверительным интервалом** (отрицательный эффект — решение снизило ошибку по факту; 95 % интервал — бутстрэп по выпускам):

   | Решение агента | Сработало | Средний эффект на MAE | 95 % интервал | Выпусков в плюс |
   |---|---|---|---|---|
   | Бустинг вместо кривой мощности (анализ → принять) | 31 | −0,040 | −0,056 … −0,024 | 23 из 31 |
   | Пересчёт в 12:00 по более свежему прогону | 31 | −0,006 | −0,015 … +0,002 | 19 из 31 |
   | Запасная модель / смена источника / флаг дрейфа | 0 | — | — | — |

   Честный вывод: выбор модели агентом — доказанная польза; пересчёт в среднем помогает, но на одном месяце не значим статистически, поэтому агент показывает фактический сдвиг каждого пересчёта, а не обещает процент. Запасные ветки в спокойном январе не понадобились — их проверяет следующий пункт.
4. **Отказоустойчивость.** Выпуск 9 февраля с намеренно испорченными входами:
   ...
   | Ветер 100 м/с в основном источнике | 48/48 ч: переход на `gfs_seamless` | 48 ч, из них 6 ч построены по мусору без предупреждения |
...
## 15. Статус реализации (на 15:40, 23.09.2026)
...
| Реплей января с бухгалтерией решений, абляция, отказоустойчивость | в репозитории (2b6c97c), `outputs/evidence/` |
| LLM-сводка с проверкой чисел, запасной LLM-провайдер | код в репозитории; нужен ключ в `.env.local` для реального прогона |
| «Спросить агента» (Амирхан — логика, Ансар — API и страница) | в работе |
